...
async function sendEmail(to, licenceKey) {
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#080a08;color:#d4dbd4;padding:40px;border-radius:12px">
          <img src="https://getarbibet.com/logo.png" alt="ArbiBet" style="width:60px;margin-bottom:24px"/>
          <h1 style="font-size:24px;color:#ffffff;margin-bottom:8px">You're in.</h1>
          ...
          <a href="https://getarbibet.com/download" style="display:inline-block;margin-top:28px;background:#00d472;color:#000;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px">
            Download ArbiBet
          </a>
...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (email) {
      ...
      // Store key — in production use a database; for now log it
